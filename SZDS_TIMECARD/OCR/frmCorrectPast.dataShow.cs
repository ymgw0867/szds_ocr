...
namespace SZDS_TIMECARD.OCR
{
    partial class frmCorrectPast
    {
        ...
        ///------------------------------------------------------------------------------------
        /// <summary>
        ///     過去応援移動票ヘッダと過去応援移動票明細のデータセットにデータを読み込む </summary>
        ///------------------------------------------------------------------------------------
        ...
        private void showItem(string hID,  GcMultiRow mr, string sYY, string sMM)
        {
            ...
            foreach (var t in dts.過去勤務票明細.Where(a => a.ヘッダID == hID).OrderBy(a => a.ID))
            {
                ...
                if (t.IsラインNull())
                {
                    mr[mRow, "lblLineNum"].Value = string.Empty;
                }
                else
                {
                    mr[mRow, "lblLineNum"].Value = t.ライン.ToString();
                }

                if (t.Is部門Null())
                {
                    mr[mRow, "lblBmnCode"].Value = string.Empty;
                }
                else
                {
                    mr[mRow, "lblBmnCode"].Value = t.部門.ToString();
                }

                if (t.Is製品群Null())
                {
                    mr[mRow, "lblHinCode"].Value = string.Empty;
                }
                else
                {
                    mr[mRow, "lblHinCode"].Value = t.製品群.ToString();
                }
