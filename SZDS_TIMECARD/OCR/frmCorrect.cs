...
namespace SZDS_TIMECARD.OCR
{
    public partial class frmCorrect : Form
    {
        ...
        /// ------------------------------------------------------------
        public frmCorrect(string dbName, string comName, string sID, bool eMode)
        {
            InitializeComponent();
            ...
            /* テーブルアダプターマネージャーに応援移動票ヘッダ、応援移動票明細テーブル、
             * 過去応援移動票ヘッダ、過去応援移動票明細テーブルアダプターを割り付ける */ 
            ouenAdp.応援移動票ヘッダTableAdapter = ohAdp;
            ouenAdp.応援移動票明細TableAdapter = omAdp;
        ...
        private void frmCorrect_Load(object sender, EventArgs e)
        {
            ...
            if (dID == string.Empty)
            {
                ...
                {
                    MessageBox.Show("勤怠データＩ／Ｐ票がありません", "勤怠データＩ／Ｐ票登録", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void CurDataUpDate(string iX)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gcMultiRow1.RowCount; i++)
                {
                    ...
                    m.ライン = Utility.NulltoStr(gcMultiRow1[i, "lblLineNum"].Value);
                    m.部門 = Utility.NulltoStr(gcMultiRow1[i, "lblBmnCode"].Value);
                    m.製品群 = Utility.NulltoStr(gcMultiRow1[i, "lblHinCode"].Value);
        ...
        private void DataDelete(int i)
        {
            ...
            try
            {
                ...
                {
                    if (System.IO.File.Exists(Properties.Settings.Default.dataPathIP + sImgNm))
                    {
                        System.IO.File.Delete(Properties.Settings.Default.dataPathIP + sImgNm);
                    }
                }
        ...
        private void textDataMake()
        {
            ...
            if (getErrData(cI, ocr)) // エラーがなかったとき
            {
                ...
                // 社員名なしの過去勤務票明細、過去応援移動票明細に社員名をセットする：2018/03/23
                Utility.getNoNameRecovery(_dbName);
        ...
        private bool chkIpOuenMatch(DataSet1 dts, out string msg)
        {
            ...
            foreach (var m in dts.応援移動票明細.Where(a => a.社員番号 != string.Empty && a.取消 == global.FLGOFF))
            {
        ...
        private void tifFileMove()
        {
            ...
            {
                // 応援移動票データを取得する
                foreach (var tu in dts.応援移動票ヘッダ.Where(a => a.年 == t.年 && a.月 == t.月 && a.日 == t.日)
                                           .OrderBy(a => a.ID))
                {
                    // 応援移動票画像ファイルパスを取得する
                    fromImg = Properties.Settings.Default.dataPathOuen + tu.画像名;

                    // 応援移動票移動先ファイルパス
                    toImg = Properties.Settings.Default.tifPath + tu.画像名;
                ...
                // 勤怠データＩ／Ｐ票画像ファイルパスを取得する
                fromImg = Properties.Settings.Default.dataPathIP + t.画像名;
        ...
        ///     
        ///     I/P勤務票と同日の応援移動票のみ対象とする
        ///------------------------------------------------------
        private void delPastOuenData()
        {
            foreach (var item in dts.勤務票ヘッダ)
            {
                int sYY = item.年;
                int sMM = item.月;
                int sDD = item.日;
                ...
                // 過去応援移動票ヘッダデータ削除
                foreach (var t in dts.応援移動票ヘッダ.Where(a => a.年 == sYY && a.月 == sMM && a.日 == sDD))
                {
                    string sBusho = t.部署コード;
        ...
        ///     対象日</param>
        ///----------------------------------------------------------------
        private void delOuenHeader(int syy, int smm, int sdd)
        {
            OleDbCommand sCom = new OleDbCommand();
            mdbControl mdb = new mdbControl();
            mdb.dbConnect(sCom);
            ...
            try
            {
                ...
                sb.Clear();
                sb.Append("delete from 応援移動票ヘッダ ");
        ...
        ///--------------------------------------------------------
        /// <summary>
        ///     応援移動票明細データ削除 </summary>
        ///--------------------------------------------------------
        private void delOuenItem()
        {
            OleDbCommand sCom = new OleDbCommand();
            mdbControl mdb = new mdbControl();
            mdb.dbConnect(sCom);
            ...
            try
            {
                ...
                sb.Clear();
                sb.Append("delete a.ヘッダID from 応援移動票明細 as a ");
                sb.Append("where not EXISTS (select * from 応援移動票ヘッダ ");
                sb.Append("WHERE 応援移動票ヘッダ.ID = a.ヘッダID)");
        ...
        private void saveLastOuenData()
        {
            try
            {
                ...
                // 過去応援移動票ヘッダデータと過去応援移動票明細データを作成します
                addLastOuendata();
            }
            catch (Exception ex)
            {
        ...
        private void addLastOuendata()
        {
            for (int i = 0; i < dts.応援移動票ヘッダ.Rows.Count; i++)
            {
                ...
                // 同日の勤怠データＩ／Ｐ票が存在しない場合は読み飛ばす
                if (!dts.勤務票ヘッダ.Any(a => a.年 == hr.年 && a.月 == hr.月 && a.日 == hr.日))
                {
                    continue;
                }
        ...
        private void gcMultiRow1_CellValueChanged(object sender, CellEventArgs e)
        {
                ...
                if (Utility.NulltoStr(gcMultiRow1[e.RowIndex, "txtShainNum"].Value) != string.Empty)
                {
                    ...
                    while (dR.Read())
                    {
                        ...
                        if (Utility.StrtoInt(val) == 0)
                        {
                            if (val.Length >= 3)
                            {
                                gcMultiRow1[e.RowIndex, "lblLineNum"].Value = val.Substring(0, 3);
                            }
                            else
                            {
                        ...
                        if (Utility.StrtoInt(val) == 0)
                        {
                            if (val.Length >= 3)
                            {
                                gcMultiRow1[e.RowIndex, "lblBmnCode"].Value = val.Substring(0, 3);
                            }
                            else
                            {
                        ...
                        if (Utility.StrtoInt(val) == 0)
                        {
                            if (val.Length >= 3)
                            {
                                gcMultiRow1[e.RowIndex, "lblHinCode"].Value = val.Substring(0, 3);
                            }
                            else
                            {
            ...
            if (e.CellName == "txtSftCode")
            {
                ...
                // 開始時間が対象シフトコードの日替わり時刻以前のときバックカラーを変更する：2018/09/05
                if (!chkChangeDayStartTime(e.RowIndex))
                {
                    // 対象のシフトコードの開始時間と異なるときバックカラーを変更する
                    // 日替わり時刻以前チェックがfalseのときのみ実行：2018/09/05
        ...
        private void lnkOuen_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            // 応援移動票データ作成
            frmOuenCorrect frmC = new frmOuenCorrect(_dbName, _comName, string.Empty, false);
            frmC.ShowDialog();
        ...
        {
            if (MessageBox.Show("表示中の勤怠データＩ／Ｐ票を削除します。よろしいですか", "削除確認", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                return;
