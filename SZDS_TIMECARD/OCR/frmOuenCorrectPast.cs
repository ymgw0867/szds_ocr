...
{
    public partial class frmOuenCorrectPast : Form
    {
        public frmOuenCorrectPast(string dbName, string comName, string sID, bool eMode)
        {
            InitializeComponent();
        ...
        DataSet1TableAdapters.過去勤務票ヘッダTableAdapter iphAdp = new DataSet1TableAdapters.過去勤務票ヘッダTableAdapter();
        DataSet1TableAdapters.過去勤務票明細TableAdapter ipmAdp = new DataSet1TableAdapters.過去勤務票明細TableAdapter();
        ...
        private void CurDataUpDate(string iX)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gcMultiRow2.RowCount; i++)
                {
                    ...
                    DataSet1.過去応援移動票明細Row m = (DataSet1.過去応援移動票明細Row)dts.過去応援移動票明細.FindByID(sID);
        ...
        private bool getErrData(string dID, OCRData ocr)
        {
            ...
            // エラーチェック実行
            if (!ocr.errCheckMainOuen(dID, dts, hArray))
            {
                return false;
            }
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            {
                MessageBox.Show("過去勤怠データＩ／Ｐ票データがありません", "勤怠データＩ／Ｐ票データ登録", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        ///-------------------------------------------------------------------------------
        /// <summary>
        ///     １．指定した過去応援移動票ヘッダデータと過去応援移動票明細データを削除する　
        ///     ２．該当する画像データを削除する</summary>
        /// <param name="i">
        ///     過去応援移動票ヘッダRow インデックス</param>
        ///-------------------------------------------------------------------------------
        private void DataDelete(int i)
        {
            ...
            {
                // ヘッダIDを取得します
                DataSet1.過去応援移動票ヘッダRow r = dts.過去応援移動票ヘッダ.Single(a => a.ID == cID[i]);

                // 画像ファイル名を取得します
                sImgNm = r.画像名;

                // データテーブルからヘッダIDが一致する過去応援移動票明細データを削除します。
                errMsg = "過去応援移動票明細データ";
                foreach (DataSet1.過去応援移動票明細Row item in dts.過去応援移動票明細.Rows)
                {
                ...
                // データテーブルから過去勤務票ヘッダデータを削除します
                errMsg = "過去応援移動票ヘッダデータ";
                r.Delete();
