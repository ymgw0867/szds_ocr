...
{
    public partial class frmOuenIndex : Form
    {
        public frmOuenIndex(string dbName, DataSet1 _dts, DataSet1TableAdapters.応援移動票ヘッダTableAdapter _hAdp, DataSet1TableAdapters.応援移動票明細TableAdapter _mAdp)
        {
            InitializeComponent();
        ...
        DataSet1TableAdapters.応援移動票明細TableAdapter mAdp = new DataSet1TableAdapters.応援移動票明細TableAdapter();
        ...
        private void GridViewShowData(DataGridView g, int sYY, int sMM, int sDD, string sBushoCode)
        {
            try
            {
                ...
                var s = dts.応援移動票ヘッダ.OrderBy(a => a.年).ThenBy(a => a.月).ThenBy(a => a.日).ThenBy(a => a.部署コード);
                ...
                foreach (DataSet1.応援移動票ヘッダRow t in s)
                {
                    g.Rows.Add();
                    ...
                    int nin = 0;
                    foreach (var m in t.Get応援移動票明細Rows())
	                {
            ...
            if (g.RowCount == 0)
            {
                MessageBox.Show("応援移動票が存在しませんでした", "データなし", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
