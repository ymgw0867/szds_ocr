...
{
    public partial class frmZanChartXls_New : Form
    {
        public frmZanChartXls_New(string dbName)
        {
            InitializeComponent();
        ...
        private void zanSumAll(int yy, int mm, int zYY, int zMM)
        {
            ...
            try
            {
                ...
                // 部署別残業計画シートの内容をCSVに出力する : 2017/10/18
                kaZanPlanSet();

                // 課別年月別に集計した人数・残業計画・生産数の2次元配列を生成する : 2017/10/18
                kaZanPlanSum();
        ...
        private void zanSum_KA(int yy, int mm, int zYY, int zMM)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < nMax; i++)
                {
                    ...
                    // シートにデータを貼り付ける
                    xlsOutPut_KA(oXls, ref oXlsBook, ref oxlsSheet, z, Properties.Settings.Default.xlsZanChart, yy, mm, zReSeizou, zRe, sdCon);
                }
            ...
            try
            {
                ...
                else if (Utility.StrtoInt(szCode) >= 2)
                {
                    ...
                    for (int i = 1; i <= zRe.GetLength(0); i++)
                    {
                        ...
                        keikaku[p - 1] += Utility.StrtoDouble(zRe[i, 6].ToString());
                    }
        ...
        ///     DataSet</param>
        ///-----------------------------------------------------------------------
        private void setZengetsuZan(ref string[,] zenArray)
        {
            var context = new CsvContext();
            ...
            var s = context.Read<Common.clsLinqCsv>(outZengetsuFile, description)
                .GroupBy(a => a.buCode.ToString().PadLeft(5, '0').Substring(0, 3))
                .Select(a => new
                {
                    sKa = a.Key,
                    zan = a.Sum(n => (n.zHayade + n.zFutsuu + n.zShinya + n.zKyuushutsu + n.zKyuShinya))
        ...
        private void linkLabel3_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (!openCsvZen)
            {
                ...
                putOuenFurikae(outZengetsuFile, zYY, zMM, 31);
            }
        ...
        private void kaZanPlanSum()
        {
            ...
            var s = context.Read<Common.clsLinqZan>(outKaZanPlanFile, description)
                .GroupBy(a => new {ka = a.buCode.ToString().Substring(0, 3), a.zYear, a.zMonth})
                .Select(n => new
                {
                    sKa = n.Key.ka,
                    sYear = n.Key.zYear,
